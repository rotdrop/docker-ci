...
class TranslatableApp {
	...
	private function createFakeFileForVueFiles() {
		...
		foreach ($this->findTranslatableFiles(['.vue']) as $vueFile) {
			...
			// t
			preg_match_all("/\Wt\s*\(\s*('\w+'|\w+),\s*'(.+)'/", $vueSource, $singleQuoteMatches);
			preg_match_all("/\Wt\s*\(\s*(\"\w+\"|\w+),\s*\"(.+)\"/", $vueSource, $doubleQuoteMatches);
			preg_match_all("/\Wt\s*\(\s*(\'\w+\'|\w+)\s*,\s*\`(.+)\`\s*\)/msU", $vueSource, $templateQuoteMatches);
			...
			// n
			preg_match_all("/\Wn\s*\(\s*('\w+'|\w+),\s*'(.+)'\s*,\s*'(.+)'\s*(.+)/", $vueSource, $singleQuoteMatches);
			preg_match_all("/\Wn\s*\(\s*(\"\w+\"|\w+),\s*\"(.+)\"\s*,\s*\"(.+)\"\s*(.+)/", $vueSource, $doubleQuoteMatches);
			preg_match_all("/\Wn\s*\(\s*(\'\w+\'|\w+)\s*,\s*\`(.+)\`\s*,\s*\`(.+)\`\s*\)/msU", $vueSource, $templateQuoteMatches);
			$matches0 = array_merge($singleQuoteMatches[0], $doubleQuoteMatches[0], $templateQuoteMatches[0]);
			$matches2 = array_merge($singleQuoteMatches[2], $doubleQuoteMatches[2], $templateQuoteMatches[2]);
